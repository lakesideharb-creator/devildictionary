...
assert.equal(window.__DEVILS_GAME__.entries.length, 45, "expanded dictionary loads");
...
const first14 = window.__DEVILS_GAME__.entries.slice(0, 14).map(entry => entry.word);
...
assert.equal(unlock.document.getElementById("completedCount").textContent, "15", "word is not collected before a verdict");
...
assert.match(unlock.document.getElementById("readerNote").textContent, /读者注释/);

const first24 = window.__DEVILS_GAME__.entries.slice(0, 24).map(entry => entry.word);
const restored = boot({ completed: first24, order: first24.slice(-5), current: null, hints: 2, rounds: 24 });
const restoredDocument = restored.document;
assert.equal(restoredDocument.getElementById("completedCount").textContent, "24");
assert.equal(restoredDocument.getElementById("coinCount").textContent, "4", "24 saved words reliably reconcile to four coins");
assert.match(restoredDocument.getElementById("stageName").textContent, /第二阶段/, "brainy stage unlocks after practice");
await swipeCurrent(restored);
assert.equal(restoredDocument.getElementById("completedCount").textContent, "24", "stage two waits for definition judgment");
restoredDocument.querySelector('.definition-choice[data-correct="false"]').click();
assert.equal(restoredDocument.getElementById("completedCount").textContent, "24", "wrong judgment never awards a coin");
restoredDocument.querySelector('.definition-choice[data-correct="true"]').click();
await sleep(690);
assert.equal(restoredDocument.getElementById("completedCount").textContent, "25");
assert.equal(restoredDocument.getElementById("coinCount").textContent, "5", "24→25 boundary earns the fifth coin");
assert.ok(restoredDocument.getElementById("coinCelebration").classList.contains("show"));

window.close();
unlock.close();
restored.close();
console.log("DOM E2E OK: stage unlock, swipe, hint, three-way judgment, reader note, progress, and coin boundaries");
